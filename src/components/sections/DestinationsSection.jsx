...
function DestinationsSection() {
  ...
  const { ref: sectionRef, visibleItems } = useStaggeredReveal(
    destinations.length,
    {
      threshold: 0.1,
      staggerDelay: 120,
  ...
  return (
    <section
      id="destinations"
      className="section-padding relative overflow-hidden bg-gradient-to-b from-white via-triply-sand/20 to-white dark:from-dark-bg dark:via-dark-surface/40 dark:to-dark-bg"
    >
      {/* Decorative elements */}
      <div
          ...
          {destinations.map((destination, index) => (
            <article
              ...
              className={`group relative overflow-hidden rounded-2xl border-2 border-triply-mint/40 dark:border-dark-border/50 bg-white dark:bg-dark-elevated shadow-lg hover:shadow-2xl dark:shadow-soft-dark dark:hover:shadow-ambient-dark transition-all duration-300 hover:-translate-y-2 hover:border-triply dark:hover:border-triply-mint ${
                visibleItems.has(index) ? "reveal-scale" : "reveal"
              }`}
            >
              ...
              <div className="space-y-3 p-6">
                ...
                {destination.price && (
                  <div className="flex items-center justify-between pt-2 border-t border-triply-mint/30 dark:border-dark-border/30">
                    <span className="text-xs text-triply-slate/60 dark:text-dark-text-secondary">
                      {t("destinations.expectedCost")}
                    </span>
                    <span className="font-display text-lg font-bold text-triply dark:text-triply-mint">
                      {destination.price}
                    </span>
                  </div>
                )}
