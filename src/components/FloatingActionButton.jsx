...
function FloatingActionButton() {
  ...
  const destinations = useMemo(() => getDestinations(t), [t]);
  ...
  const destinationKeywords = useMemo(() => {
    // خريطة المرادفات لكل وجهة (تدعم العربية والإنجليزية)
    const synonymMap = {
      london: [
        "london",
        "uk",
      ...
      paris: ["paris", "france", "فرنسا", "باريس", "فرنسا"],
      cairo: ["cairo", "egypt", "مصر", "القاهرة", "قاهرة"],
      istanbul: ["istanbul", "turkey", "تركيا", "إسطنبول", "اسطنبول"],
      dubai: ["dubai", "uae", "الامارات", "الإمارات", "دبي", "emirates"],
    };

    return destinations.map((destination) => {
      const city = destination.name.split(" - ")[0].toLowerCase();
      const country = destination.name.toLowerCase();
      const destKey = destination.key || "london";
      const extras = synonymMap[destKey] ?? [];
      return {
        destination,
        keywords: [city, country, ...extras.map((term) => term.toLowerCase())],
      };
    });
  }, [destinations]);
  ...
  const generateBotResponse = (rawMessage) => {
    ...
    if (
      greetings.some(
        (term) => normalized.startsWith(term) || normalized.includes(`${term} `)
      )
    ) {
    ...
    const tripDuration = extractNumber(
      normalized,
      /(\d+)\s*(?:يوم|ايام|أيام|day|ليله|ليالي|night)/
    );
    const budgetAmount = extractNumber(
      normalized,
      /(\d+(?:[\.,]\d+)?)\s*(?:ريال|sar|rs|رس)/
    );

    const insights = [];
    if (groupSize) {
      insights.push(
        `نسق الخطط لعدد ${groupSize} مشارك بسهولة، ونقسّمهم على حسب الاهتمامات.`
      );
    }
    if (tripDuration) {
      insights.push(
        `المدة ${tripDuration} أيام تتيح دمج أنشطة صباحية ومسائية متوازنة.`
      );
    }
    if (budgetAmount) {
      insights.push(
        `نراعي ميزانية تقريبية تبلغ ${budgetAmount.toLocaleString(
          "ar-EG"
        )} ريال ونقترح باقات مناسبة.`
      );
    }
    ...
    const scoredTopic = topicBank
      .map((entry) => ({
        ...
        score: entry.keywords.reduce(
          (score, keyword) =>
            tokens.some((token) => token.includes(keyword)) ? score + 1 : score,
          0
        ),
      }))
  ...
  return (
    <div className="fixed bottom-6 left-4 sm:left-6 z-50 flex flex-col items-start gap-4">
        ...
        >
          <div className="flex items-center justify-between bg-gradient-to-r from-triply-mint/80 via-white to-triply-sand/70 px-4 py-3 dark:from-triply-teal/50 dark:via-slate-900/90 dark:to-triply-sand/30">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-triply-slate/70 dark:text-triply-sand/80">
          ...
          <form
            ...
            className="border-t border-triply-mint/30 bg-white/40 px-3 py-3 dark:bg-slate-900/80 dark:border-triply-teal/30"
          >
      ...
      <button
        ...
        aria-label={
          isOpen ? t("chatbot.hideAssistant") : t("chatbot.openAssistant")
        }
      >
        <svg
        ...
        >
          ...
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M9.5 15s1 .75 2.5.75S14.5 15 14.5 15"
          />
        </svg>
        {!isOpen && (
